...
export default function BookDemoPageClient() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // You would combine countryCode and phone here
  ...
  const logos = [
    ...
    { name: "Snowflake", icon: (
      <svg className="w-5 h-5 mr-1.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
        <path d="M12 2v20M2 12h20M12 2l3 3M12 2l3 3M12 22l-3-3M2 12l3 3M22 12l-3-3M5 5l14 14M19 5L5 19" strokeLinecap="round"/>
      </svg>
    )},
    { name: "Flash", icon: (
      <svg className="w-5 h-5 mr-1.5" viewBox="0 0 24 24" fill="currentColor">
                  ...
                  >
                    ...
                    <div className="flex flex-col gap-1.5">
                      ...
                      <div className="relative flex items-center">
                        <div className="absolute left-1 top-1 bottom-1 flex items-center justify-center pl-3 pr-2 border-r border-zinc-200 cursor-pointer group hover:bg-zinc-50 rounded-l-xl transition-colors">
                          <span className="text-lg leading-none">{countryOptions.find(c => c.code === countryCode)?.flag || "🇮🇳"}</span>
                          <ChevronDown className="w-3.5 h-3.5 ml-1 text-zinc-500 group-hover:text-zinc-800" />
                          <select
                            value={countryCode}
                        ...
                        <input
                          ...
                          placeholder="(334) 343-4333"
                          className="w-full pl-[5.5rem] pr-4 py-3 rounded-xl border border-zinc-200 bg-white text-zinc-900 text-sm font-medium focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500 transition-colors shadow-xs"
                        />
                      </div>
                    </div>
